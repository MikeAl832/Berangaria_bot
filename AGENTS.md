...
- Normal Muse chat uses `temperature: 1.0` with `reasoning.effort: low`. Reasoning is
  mandatory for Muse Spark 1.3; supported levels are `minimal` through `max`. Keep the
  shipped request surface to `temperature` plus `reasoning` unless an equivalent live A/B
  justifies another sampling control. Summarization sends `reasoning.effort: high` on its
  own request.
...
- Judge prompt or sampling changes with equivalent real-dialogue A/B trials. Passing
  unit tests proves wiring and invariants, not conversational quality.
...
.venv/bin/bandit -q -ll -r . -x ./.venv,./venv,./tests
git diff --check
```
...
- Commands or jobs that clear, summarize, replace, or delete history must use the same turn lock. Reactions also use it before changing history.
...
- New persisted prompt rows carry `provider_sent: false`. Immediately before the first
  ...
  row has been sent it must be appended as a structured tail event so the cached prompt
...
- When a streamed assistant tool call includes `reasoning_details`, concatenate the
  structured chunks in provider order and echo the full sequence unmodified on the next
  tool round. Prefer it over a duplicate flattened `reasoning_content` field.
...
- Every chat scope uses one stable opaque conversation id, sent to OpenRouter as
  `session_id` / `x-session-id` plus `provider.only: ["meta"]`, disabled fallbacks, and
...
- `telegram.error.Conflict` on getUpdates is fatal: log CRITICAL (and flush), notify
  ...
  for 20 minutes, `os._exit(1)`. Docker HEALTHCHECK (`scripts/loop_healthcheck.py`) SIGKILLs the berangaria
  worker (Compose `init: true` — not PID 1, which ignores kill) if
  `/data/loop_heartbeat` is older than 25 minutes so a GIL freeze still recovers
  via `restart: always`.

...
Примеры (внутри контейнера бота):
    docker compose exec bot python scripts/sticker_search.py "недоумение, кто-то сморозил глупость"
    docker compose exec bot python scripts/sticker_search.py --top 15 "ржу в голос"

С хоста:
    QDRANT_HOST=localhost python scripts/sticker_search.py "одобряю, огонь"
"""
...
def main():
    ...
    from berangaria.stickers.store import search_stickers, collection_count, STICKER_MIN_SCORE
